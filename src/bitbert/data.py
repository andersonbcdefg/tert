...
from .util import chunk_text, dict_slice, send_to_device
...
class InterleavedDataset(IterableDataset):
  def __init__(
    ...
    split_text_at: int | None = None,  # how many words to split the text into chunks of
    infinite: bool = False,
  ):
    ...
    while True:
        ...
        while remaining_indices:
          ...
          try:
            ...
            if self.infinite:
              ...
              iterators[selected_index] = iter(self.datasets[selected_index])
      ...
      try:
        while True:
          ...
          chunks = [{"text": chunk} for chunk in text_chunks if len(chunk) > 50]
        ...
        print("After exhausting all datasets, the count of each is:", self.counts)
        break
...
def get_combined_dataset(
  ...
  return InterleavedDataset(datasets, weights, split_text_at=chunk_size, infinite=True)
...
def get_interleaved_dataloader(
  ...
  interleaved = InterleavedDataset(
    [fw, dclm, gutenberg, wiki], [9_672_200, 10_000_000, 8_000_000, 226_000]
  )
