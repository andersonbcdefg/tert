import torch
from .util import send_to_device
...
def get_wsd_scheduler(
    ...
    def lr_lambda(step):
        ...
        if step < total_steps - final_steps:
            ...
            return 1.0 + (final_scale - 1.0) * decay_progress
...
def get_constant_scheduler(optimizer, total_steps, warmup_frac=0.05):
    # it's wsd with no decay
    return get_wsd_scheduler(optimizer, total_steps, warmup_frac, 0.0)
...
class BatchSizeSchedule:
    ...
    def __init__(
        ...
        self.steps_per_batch_size = max(
            1, int(round(warmup_optimizer_steps / num_levels))
        )
