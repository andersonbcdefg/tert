...
class IndexFirstAxisResidual(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output, grad_residual):  # pyright: ignore
        (indices,) = ctx.saved_tensors
        assert grad_output.ndim >= 2
        other_shape = grad_output.shape[1:]
...
def document_ids_from_cu_seqlens(cu_seqlens):
    ...
    Args:
    cu_seqlens (torch.Tensor): Cumulative sequence lengths tensor of shape (num_docs + 1,)

    Returns:
    torch.Tensor: Document IDs tensor of shape (total_tokens,)
    """
...
def get_block_mask(cu_seqlens: torch.Tensor) -> BlockMask:
